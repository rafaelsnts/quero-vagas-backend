...
export const addFormacaoAcademica = async (req, res) => {
  ...
  try {
    ...
    console.error("Erro ao adicionar formação acadêmica:", error);
...
export const deleteFormacaoAcademica = async (req, res) => {
  ...
  try {
    ...
    if (!formacao) {
      return res
        .status(404)
        .json({ message: "Formação acadêmica não encontrada." });
    }

    if (formacao.candidatoId !== candidatoId) {
      return res.status(403).json({
        message: "Você não tem permissão para excluir esta formação.",
      });
    }

    await prisma.formacaoAcademica.delete({
      where: { id: formacaoId },
    });

    res.status(204).send();
  } catch (error) {
    console.error("Erro ao deletar formação acadêmica:", error);
...
export const updateFormacaoAcademica = async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ message: "Erro ao atualizar formação." });
  }
};
export const uploadCurriculo = async (req, res) => {
  ...
  try {
    const perfil = await prisma.perfil.upsert({
      where: { userId },
      update: { curriculoUrl },
      create: { userId, curriculoUrl },
    });
    res.status(200).json({ message: "Currículo enviado com sucesso!", perfil });
  } catch (error) {
    res.status(500).json({ message: "Erro ao salvar o caminho do currículo." });
  }
};
